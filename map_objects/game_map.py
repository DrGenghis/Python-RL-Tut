...
class GameMap:
	...
	def place_entities(self, room, entities):
		...
		monster_chances = {
			'orc': 80,
			'troll': from_dungeon_level([[15, 30], [30, 5], [60, 7]], self.dungeon_level)
		}
		...
		for i in range(number_of_monsters):
			...
			if not any([entity for entity in entities if entity.x and entity.y == y]):
				monster_choice = random_choice_from_dict(monster_chances)
				if monster_choice == 'orc':
					fighter_component = Fighter(hitdice=8, base_damage='1d2', brawn=2, finesse=1, allure=0, vitality=2,xp=35)
					ai_component = BasicMonster()
					
					monster = Entity(x, y, 'o', libtcod.desaturated_green, 'orc', blocks=True,
										render_order=RenderOrder.ACTOR, fighter=fighter_component, ai=ai_component)
				else:
					fighter_component = Fighter(hitdice=12, base_damage='2d3', brawn=4, finesse=2, allure=0, vitality=4, xp=100)
					ai_component = BasicMonster()
		...
		for i in range(number_of_items):
			...
			if not any([entity for entity in entities if entity.x == x and entity.y == y]):
				...
				if item_chance == 'healing_potion':
					...
					item = Entity(x, y, '#', libtcod.red, 'Scroll of Fireball', render_order=RenderOrder.ITEM, item=item_component)
				elif item_chance == 'lightning_scroll':
					item_component = Item(use_function=cast_lightning, damage=40, maximum_range=5)
					item = Entity(x, y, '#', libtcod.yellow, 'Scroll of Lightning Bolt', render_order=RenderOrder.ITEM, item=item_component)
					...
					item = Entity(x, y, '#', libtcod.light_pink, 'Scroll of Confuse Monster', render_order=RenderOrder.ITEM, item=item_component)
				# Equipment
